...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      // Only process OAuth URL tokens on the callback page.
      // Running it on /login or /signup causes timeouts from stale OAuth fragments.
      if (isCallbackPage) {
        try {
        ...
        // Clear stale OAuth artifacts from URL on non-callback pages
        authService.clearOAuthArtifactsFromUrl();
      }

      let currentUser = await authService.getCurrentUser();
      
      // If user is null on callback page, wait and retry — Supabase's
      // internal listener may still be processing the code exchange
      if (!currentUser && isCallbackPage) {
