...
const handleList = async (req, res) => {
  ...
  let query = supabase
    .from('jobs')
    .select(
      'id,user_id,title,company,platform,job_description,budget_min,budget_max,currency,proposed_price,status,followup_date,followup_at,applied_at,closed_at,created_at,notes,proposal',
      { count: 'estimated' }
    )
    .eq('user_id', user.id);
